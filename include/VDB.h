...
/// @file VDB.h
/// @brief VDB class in this file handles the loading, drawing and attribute handling of any OpenVDB file
/// loaded in
/// @author Callum James
/// @version 1.0
/// @date 12/02/2014
...
/// @class VDB
/// @brief VDB class handles the loading of a VDB file. Separates the attributes out and makes them
/// accessible to other classes. Handles the templates used within the OpenVDB library as well as
/// all the drawing of the different elements to the file. Adapts to files of different number of
/// channels data. This is where all the OpenVDB specific is done.
class VDB
{
	public:
		...
		/// @brief Return the channel extremes of the current channel - returns BBoxBare
		inline BBoxBare getCurrentChannelExtremes() {return m_channelExtremes->at( m_currentActiveChannelPoints );}
		/// @brief Return the channel extremes of the specified channel - returns BBoxBare
		/// @param [in] _channel int - the channel to query
		inline BBoxBare getChannelExtremesAt( int _channel ) {return m_channelExtremes->at( _channel );}
		...
		inline void setVectorColour( openvdb::Vec3f _colour ) {m_vectorColour = _colour;}
		...
		/// @brief Get the value s used for LOD at the current active channel - returns float
		float getS();
		/// @brief Get the value s used for LOD at the specified channel - returns float
		...
		void buildBBox( float _minx, float _maxx,
		...
		/// @param [in] _index int - the crop box to set
		void setCrop( openvdb::Vec3f _centre, float _w, float _h, float _d, int _index );
		...
		/// @param [in] _index int - the crop box to set
		inline void setCropCentre( openvdb::Vec3f _c, int _index ) {m_crop[_index].setCentre( _c );}
		/// @brief Set Crop box centre at index
		/// @param [in] _x float - x value of centre
		/// @param [in] _y float - y value of centre
		/// @param [in] _z float - z value of centre
		/// @param [in] _index int - the crop box to set
		inline void setCropCentre( float _x, float _y, float _z, int _index ) {m_crop[_index].setCentre( _x, _y, _z );}
		...
		/// @param [in] _index int - the crop box to set
		inline void translateCrop( openvdb::Vec3f _t, int _index ) {m_crop[_index].translate( _t );}
		/// @brief Method to colour the specified crop
		/// @param [in] _colour openvdb::vec3f - the colour to set
		/// @param [in] _index int - the crop box to set
		inline void setCropColour( openvdb::Vec3f _colour, int _index ) {m_crop[_index].setColour( _colour );}
		/// @brief Get the colour of the specified crop box - returns openvdb::Vec3f
		/// @param [in] _index int - the crop box to query
		inline openvdb::Vec3f cropColour( int _index ) {return m_crop[_index].colour();}
		/// @brief Get stored available GPU memory in KB - returns GLint
		inline GLint getCurrentAvailableGPUMemKB() {return m_current_available_GPU_mem_kb;}
		...
		inline std::string channelName( int _channel ) {return m_variableNames[_channel];}
		/// @brief Return the specified channel type - returns std::string
		/// @param [in] _channel int - channel to query
		inline std::string channelType( int _channel ) {return m_variableTypes[_channel];}
		/// @brief Return the grid dimensions from the specified channel - returns openvdb::Coord
	...
	private:
		...
		/// @param [in] _grid typename GridType::ConstPtr - the grid to retrieve values from
		...
		/// @param [in] _grid typename GridType::Ptr - the grid to retrieve values from
		template <typename GridType>
		...
		BoundBox m_crop[3];
		// used when scans are run so that the crop box can be returned to its previous state before it was run
		...
		/// @brief Vector storing the channel value data to upload to a texture buffer on the GPU
		...
		void pushBackVDBVert( std::vector<vDat>* _v, openvdb::Vec3f _point, vDat _vert );
