...
/// @brief ShaderLibrary class to store and switch all regsitered and compiled shaders
/// @author Callum James
/// @version 1.0
/// @date 12/02/2014
/// Revision History:
...
/// @class ShaderLibrary
/// @brief ShaderLibrary class to store all compiled and linked shaders and handle access to them
/// for the setting of attributes and uniforms. Also allows the switching of the currently actively shader
class ShaderLibrary
{
	public:
		...
		// create a new shader family and attach a vert and frag shader named correctly
		/// @brief Method to create a family with an autromatic vertex and fragment shader
		...
		void loadVertFragShaderFile( std::string _shaderName, std::string _vertex, std::string _frag );
		...
		void loadVertFragShaderSource( std::string _shaderName, char* _vertex, char* _frag );
		...
		void setShaderUniformFromMat3( const std::string& _uniformName, openvdb::Mat3R _m );
		/// @brief Upload Matrix 4x4 uniform to shaders
		/// @param [in] _uniformName const std::string - name of uniform
		/// @param [in] _m openvdb::Mat4s - matrix to upload
		void setShaderUniformFromMat4( const std::string& _uniformName, openvdb::Mat4s _m );
		...
		void setShaderParam3f( const std::string& _paramName, float _f1, float _f2, float _f3 );
		...
		void setShaderParam4f( const std::string& _paramName, float _f1, float _f2, float _f3, float _f4 );
		...
		void setShaderParam4i( const std::string& _paramName, int _i1, int _i2, int _i3, int _i4 );
	...
	private:
		...
		/// @brief Method to return a suffix from the shader type to keep internal naming conventions - returns std::string
